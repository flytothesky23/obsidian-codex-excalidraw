...
export function buildCanvasBrief(notes: NoteContext[], targetPath: string): string {
  ...
  return [
    ...
    "",
    "- Edit only the target `.canvas` JSON file.",
    ...
    "- Use `x`, `y`, `width`, and `height` to make the layout readable without overlap.",
    ...
    "- If the canvas is harder to read than the note, simplify the node count and improve labels.",
    "",
    "# Output Standard",
    "",
