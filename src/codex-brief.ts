...
function svgSystemModeSections(params: {
  ...
  return {
    thinking: [
      ...
      "- Asset pass: use the prepared Excalidraw library cache as the first-class asset source. Pick source-specific editable library items before drawing primitive glyphs yourself.",
      "- If external or vault-local assets are not available, draw simplified editable glyphs yourself only after naming the missing asset category. Do not paste screenshots or flatten the result into one image.",
      ...
      "- Connection pass: choose one primary flow direction and route arrows through gutters with orthogonal/elbow connectors. Arrows must mean causal flow, data/control flow, dependency, decision, verification, or feedback.",
      ...
      "- Verification pass: before finishing, inspect the drawing JSON and fix title drift, text overflow, arrows through labels, crossed connectors, disconnected nodes, and decorative shapes without meaning.",
    ],
    budget: [
      ...
      "- Use 5-9 semantic nodes plus 3-8 small editable glyph/icon clusters. At least three glyph/icon clusters should come from cached library items when matching items exist.",
      "- Use 1-3 visible zones/containers and 2-4 callouts or caption tags.",
      "- Use 5-10 arrows. Every arrow should have an obvious reason; label only ambiguous relationships.",
      `- Minimum readable text size ${params.minBodyFont} for body text and ${params.minTitleFont} for the main title.`,
    ...
    requirements: [
      ...
      "- When prepared `.excalidrawlib` cache paths are listed, open those JSON files and copy suitable editable library item elements into the drawing. Use primitive glyphs only as fallback.",
      ...
      "- Avoid arrows crossing through labels or other arrows. Route arrows around boxes with orthogonal elbows, lane gutters, or small junction nodes.",
      "- Include source-specific wikilinks only when they are useful labels, not as raw path clutter.",
      "- In the final response, report counts for zones, library items used, fallback glyphs, semantic nodes, arrows, callouts, and whether overlap/text overflow/crossed connectors were checked.",
    ],
  };
}
